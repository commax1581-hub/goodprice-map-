...
순서(버그이력 #58):
 ...
 2) 캐시에 이름이 없으면 원문 시군구 → regions.codes_of(시군구, 힌트=시도)
    옛 이름 하나가 새 구 여럿(인천 중구·서구)이면 주소의 읍면동으로 가른다(INCHEON_SPLIT)
...
# 인천 행정체제 개편(2026-06-30): 옛 중구·서구는 읍면동(없으면 검단 안에만 있는 도로명)으로 새 구를 가른다.
...
def attach(rows, check=None):
    ...
    for r in rows:
        ...
        if not code:
            ...
            if len(got) == 1:
                code, src = got[0], '원문이름'
            elif (sd, g) in INCHEON_SPLIT:
                pat, hit, other = INCHEON_SPLIT[(sd, g)]
                code, src = (hit if re.search(pat, r['주소'] + ' ' + r.get('행정동', '')) else other), '원문이름+읍면동'
        if not code:
            src = '못정함'
        r['시군구코드'] = code
