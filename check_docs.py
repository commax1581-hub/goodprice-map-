...
rows = {int(m.group(1)): m.group(2) for m in re.finditer(r'^\| (\d+) \| [^|]+\| ([^|]+)\|', summary_part, re.M)}
details = {int(n) for n in re.findall(r'^### (\d+)\. ', bug_text, re.M)}
for n in sorted(set(rows) - details): FAIL(f'버그 #{n}: 요약 표에는 있는데 상세 절(### {n}.)이 없음')
for n in sorted(details - set(rows)): FAIL(f'버그 #{n}: 상세 절은 있는데 요약 표 행이 없음')

def bug_refs(text):
    refs = set()
    for m in re.finditer(r'#(\d{1,3})(?![0-9A-Za-z])(?:\s*~\s*#?(\d{1,3})(?![0-9A-Za-z]))?', text):
